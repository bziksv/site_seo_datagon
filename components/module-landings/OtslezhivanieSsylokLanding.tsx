...
import {
  ...
  LINK_TRACK_HERO,
  LINK_TRACK_INSIGHTS,
  LINK_TRACK_INSIGHTS_GRID,
  LINK_TRACK_INSIGHTS_HIGHLIGHT,
  LINK_TRACK_INSIGHTS_OUTCOMES,
  LINK_TRACK_OPTIONS,
  LINK_TRACK_PLAIN,
  LINK_TRACK_SCREENSHOTS,
  LINK_TRACK_SLICES,
  LINK_TRACK_STATS,
  LINK_TRACK_STEPS,
  LINK_TRACK_CHECKS,
  LINK_TRACK_TECH_LAYERS,
  LINK_TRACK_TECH_SECTION,
  LINK_TRACK_VIDEOS,
} from "@/lib/content/otslezhivanie-ssylok-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function OtslezhivanieSsylokLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : LINK_TRACK_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {LINK_TRACK_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {LINK_TRACK_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{LINK_TRACK_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{LINK_TRACK_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={LINK_TRACK_SCREENSHOTS[0].src}
                      alt={LINK_TRACK_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={LINK_TRACK_TECH_SECTION}
        layers={LINK_TRACK_TECH_LAYERS}
        titleId="link-track-tech-title"
      />
      ...
      <div className="min-w-0 max-w-full overflow-x-clip bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От проекта до уведомления
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {LINK_TRACK_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            ...
            <p className="mt-3 max-w-2xl text-slate-600">
              SEO, закупка ссылок и контекстная реклама — один инструмент разбора.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {LINK_TRACK_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={LINK_TRACK_INSIGHTS}
            grid={LINK_TRACK_INSIGHTS_GRID}
            highlight={LINK_TRACK_INSIGHTS_HIGHLIGHT}
            outcomes={LINK_TRACK_INSIGHTS_OUTCOMES}
            titleId="link-track-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Проекты, форматы ввода и проверки — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {LINK_TRACK_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">При ежедневном обходе страницы донора.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {LINK_TRACK_CHECKS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Ссылки и тех-SEO в одной платформе.</p>
