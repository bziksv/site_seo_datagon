...
import {
  ...
  WORD_GEN_HERO,
  WORD_GEN_INSIGHTS,
  WORD_GEN_INSIGHTS_GRID,
  WORD_GEN_INSIGHTS_HIGHLIGHT,
  WORD_GEN_INSIGHTS_OUTCOMES,
  WORD_GEN_OPTIONS,
  WORD_GEN_PLAIN,
  WORD_GEN_SCREENSHOTS,
  WORD_GEN_SLICES,
  WORD_GEN_STATS,
  WORD_GEN_STEPS,
  WORD_GEN_TECH_LAYERS,
  WORD_GEN_TECH_SECTION,
} from "@/lib/content/generator-slov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function GeneratorSlovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : WORD_GEN_FEATURES;
  ...
  return (
    <>
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{WORD_GEN_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{WORD_GEN_HERO.sub}</p>
              ...
              <div className="mt-10">
                <ModuleLeadCta
                  variant="hero"
                  idPrefix="word-gen-hero"
                  title="Получить комбинации"
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={WORD_GEN_SCREENSHOTS[0].src}
                      alt={WORD_GEN_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={WORD_GEN_TECH_SECTION}
        layers={WORD_GEN_TECH_LAYERS}
        titleId="word-gen-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От блоков слов до списка фраз
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {WORD_GEN_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Режимы генерации</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре направления настройки — от полного ядра до гео и естественных формулировок.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {WORD_GEN_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={WORD_GEN_INSIGHTS}
            grid={WORD_GEN_INSIGHTS_GRID}
            highlight={WORD_GEN_INSIGHTS_HIGHLIGHT}
            outcomes={WORD_GEN_INSIGHTS_OUTCOMES}
            titleId="word-gen-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Блоки, опции и результат — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {WORD_GEN_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Дополнительные опции</h2>
            <p className="mt-2 text-sm text-slate-600">Тонкая настройка перед «Получить комбинации».</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {WORD_GEN_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Семантика от генерации до публикации.</p>
          ...
          <ModulePlainSection data={WORD_GEN_PLAIN} titleId="word-gen-plain-title" />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Почему {SITE.name}</h2>
            <div className="mt-8 grid gap-8 md:grid-cols-3">
              {WORD_GEN_ADVANTAGES.map((a) => (
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы собрать семантическое ядро?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — генератор ключевых слов и остальные инструменты платформы в одном
                месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="word-gen-footer"
