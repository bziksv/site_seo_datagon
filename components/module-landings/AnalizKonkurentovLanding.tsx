...
import {
  ...
  COMPETITOR_HERO,
  COMPETITOR_INSIGHTS,
  COMPETITOR_INSIGHTS_GRID,
  COMPETITOR_INSIGHTS_HIGHLIGHT,
  COMPETITOR_INSIGHTS_OUTCOMES,
  COMPETITOR_OPTIONS,
  COMPETITOR_PLAIN,
  ...
  COMPETITOR_STATS,
  COMPETITOR_STEPS,
  COMPETITOR_TECH_LAYERS,
  COMPETITOR_TECH_SECTION,
} from "@/lib/content/analiz-konkurentov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function AnalizKonkurentovLanding({ module }: Props) {
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {COMPETITOR_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {COMPETITOR_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{COMPETITOR_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{COMPETITOR_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[4/3] min-h-[240px] w-full sm:min-h-[300px] lg:min-h-[360px] xl:min-h-[400px]">
                    <Image
                      src={COMPETITOR_SCREENSHOTS[0].src}
                      alt={COMPETITOR_SCREENSHOTS[0].caption}
                      fill
                  ...
                  <figcaption className="border-t border-slate-200 bg-slate-50 px-4 py-2.5 text-center text-xs font-medium text-slate-600">
                    {COMPETITOR_SCREENSHOTS[0].caption}
                  </figcaption>
                </figure>
              </HeroParallaxMedia>
            </div>
      ...
      <ModuleTechSection
        section={COMPETITOR_TECH_SECTION}
        layers={COMPETITOR_TECH_LAYERS}
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От ключей до карты конкурентов
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {COMPETITOR_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Четыре среза конкурентной выдачи</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              По каждому ключу — ТОП, мета, структура URL и сводные метрики по доменам. Всё в одном отчёте,
              без ручного сбора в таблицы.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {COMPETITOR_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={COMPETITOR_INSIGHTS}
            grid={COMPETITOR_INSIGHTS_GRID}
            highlight={COMPETITOR_INSIGHTS_HIGHLIGHT}
            outcomes={COMPETITOR_INSIGHTS_OUTCOMES}
            titleId="competitor-insights-title"
          />
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Настройки проверки</h2>
            <p className="mt-2 text-sm text-slate-600">Регион, глубина ТОПа и удобная работа с матрицей.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {COMPETITOR_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">
              После карты конкурентов — углубитесь в позиции, релевантность и мета.
            </p>
          ...
          <ModulePlainSection data={COMPETITOR_PLAIN} titleId="competitor-plain-title" />
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы разобрать конкурентов по ключам?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — анализ конкурентов и остальные SEO-модули платформы в одном
                месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="competitor-footer"
