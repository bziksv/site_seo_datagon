...
import {
  UNIQUE_WORDS_ADVANTAGES,
  UNIQUE_WORDS_FAQ,
  UNIQUE_WORDS_FEATURES,
  UNIQUE_WORDS_HERO,
  UNIQUE_WORDS_INSIGHTS,
  UNIQUE_WORDS_INSIGHTS_GRID,
  UNIQUE_WORDS_INSIGHTS_HIGHLIGHT,
  UNIQUE_WORDS_INSIGHTS_OUTCOMES,
  UNIQUE_WORDS_OPTIONS,
  UNIQUE_WORDS_PLAIN,
  UNIQUE_WORDS_SCREENSHOTS,
  UNIQUE_WORDS_SLICES,
  UNIQUE_WORDS_STATS,
  UNIQUE_WORDS_STEPS,
  UNIQUE_WORDS_TABLE_ACTIONS,
  UNIQUE_WORDS_TECH_LAYERS,
  UNIQUE_WORDS_TECH_SECTION,
  UNIQUE_WORDS_VIDEOS,
} from "@/lib/content/vydelenie-unikalnykh-slov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function VydelenieUnikalnykhSlovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : UNIQUE_WORDS_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {UNIQUE_WORDS_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {UNIQUE_WORDS_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{UNIQUE_WORDS_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{UNIQUE_WORDS_HERO.sub}</p>
              ...
              <div className="mt-10">
                <ModuleLeadCta
                  variant="hero"
                  idPrefix="unique-words-hero"
                  title="Разобрать фразы"
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={UNIQUE_WORDS_SCREENSHOTS[0].src}
                      alt={UNIQUE_WORDS_SCREENSHOTS[0].caption}
                      fill
                  ...
                  <figcaption className="border-t border-slate-200 bg-slate-50 px-4 py-2.5 text-center text-xs font-medium text-slate-600">
                    {UNIQUE_WORDS_SCREENSHOTS[0].caption}
                  </figcaption>
                </figure>
              </HeroParallaxMedia>
            </div>
      ...
      <ModuleTechSection
        section={UNIQUE_WORDS_TECH_SECTION}
        layers={UNIQUE_WORDS_TECH_LAYERS}
        titleId="unique-words-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От вставки фраз до таблицы слов
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {UNIQUE_WORDS_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            ...
            <p className="mt-3 max-w-2xl text-slate-600">
              Семантика, конкуренты, копирайтинг и маркетинг — один инструмент разбора.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {UNIQUE_WORDS_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={UNIQUE_WORDS_INSIGHTS}
            grid={UNIQUE_WORDS_INSIGHTS_GRID}
            highlight={UNIQUE_WORDS_INSIGHTS_HIGHLIGHT}
            outcomes={UNIQUE_WORDS_INSIGHTS_OUTCOMES}
            titleId="unique-words-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Ввод фраз, таблица результата и экспорт — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {UNIQUE_WORDS_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Действия в таблице результата</h2>
            <p className="mt-2 text-sm text-slate-600">После обработки — с каждой строкой отчёта.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {UNIQUE_WORDS_TABLE_ACTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Списки, текст и семантика в одной платформе.</p>
          ...
          <ModulePlainSection data={UNIQUE_WORDS_PLAIN} titleId="unique-words-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: выделение уникальных слов"
              lead="Как вставить фразы и работать с таблицей результата."
              items={UNIQUE_WORDS_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Нужен разбор фраз на слова?</h2>
