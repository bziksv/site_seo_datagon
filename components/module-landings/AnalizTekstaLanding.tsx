...
import {
  TEXT_ANAL_ADVANTAGES,
  TEXT_ANAL_FAQ,
  TEXT_ANAL_FEATURES,
  TEXT_ANAL_HERO,
  TEXT_ANAL_INSIGHTS,
  TEXT_ANAL_INSIGHTS_GRID,
  TEXT_ANAL_INSIGHTS_HIGHLIGHT,
  TEXT_ANAL_INSIGHTS_OUTCOMES,
  TEXT_ANAL_OPTIONS,
  TEXT_ANAL_PLAIN,
  TEXT_ANAL_SCREENSHOTS,
  TEXT_ANAL_SLICES,
  TEXT_ANAL_STATS,
  TEXT_ANAL_STEPS,
  TEXT_ANAL_CHECKS,
  TEXT_ANAL_TECH_LAYERS,
  TEXT_ANAL_TECH_SECTION,
  TEXT_ANAL_VIDEOS,
} from "@/lib/content/analiz-teksta-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function AnalizTekstaLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : TEXT_ANAL_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {TEXT_ANAL_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {TEXT_ANAL_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{TEXT_ANAL_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{TEXT_ANAL_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={TEXT_ANAL_SCREENSHOTS[0].src}
                      alt={TEXT_ANAL_SCREENSHOTS[0].caption}
                      fill
                  ...
                  <figcaption className="border-t border-slate-200 bg-slate-50 px-4 py-2.5 text-center text-xs font-medium text-slate-600">
                    {TEXT_ANAL_SCREENSHOTS[0].caption}
                  </figcaption>
                </figure>
              </HeroParallaxMedia>
            </div>
      ...
      <ModuleTechSection
        section={TEXT_ANAL_TECH_SECTION}
        layers={TEXT_ANAL_TECH_LAYERS}
        titleId="text-anal-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От вставки текста до отчёта
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {TEXT_ANAL_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            ...
            <p className="mt-3 max-w-2xl text-slate-600">
              SEO, копирайтинг и e-commerce — один инструмент проверки контента.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {TEXT_ANAL_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={TEXT_ANAL_INSIGHTS}
            grid={TEXT_ANAL_INSIGHTS_GRID}
            highlight={TEXT_ANAL_INSIGHTS_HIGHLIGHT}
            outcomes={TEXT_ANAL_INSIGHTS_OUTCOMES}
            titleId="text-anal-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Ввод, настройки, таблица и график Ципфа — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {TEXT_ANAL_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">При анализе вставленного текста или страницы по URL.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {TEXT_ANAL_CHECKS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Текст, семантика и релевантность — в одной платформе.</p>
          ...
          <ModulePlainSection data={TEXT_ANAL_PLAIN} titleId="text-anal-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: анализ текста"
              lead="Как запустить проверку и читать таблицу результата."
              items={TEXT_ANAL_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Нужен анализ перед публикацией?</h2>
