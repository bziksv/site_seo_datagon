...
import {
  ...
  META_MON_HERO,
  META_MON_INSIGHTS,
  META_MON_INSIGHTS_GRID,
  META_MON_INSIGHTS_HIGHLIGHT,
  META_MON_INSIGHTS_OUTCOMES,
  META_MON_OPTIONS,
  META_MON_PLAIN,
  META_MON_SCREENSHOTS,
  META_MON_SLICES,
  META_MON_STATS,
  META_MON_STEPS,
  META_MON_TECH_LAYERS,
  META_MON_TECH_SECTION,
  META_MON_VIDEOS,
} from "@/lib/content/proverka-meta-tegov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function ProverkaMetaTegovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : META_MON_FEATURES;
  ...
  return (
    <>
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{META_MON_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{META_MON_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={META_MON_SCREENSHOTS[0].src}
                      alt={META_MON_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={META_MON_TECH_SECTION}
        layers={META_MON_TECH_LAYERS}
        titleId="meta-mon-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От списка URL до отчёта по мета-тегам
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {META_MON_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Что проверяем в отчёте</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре группы полей — от сниппета в поиске до canonical и пакетной проверки.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {META_MON_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={META_MON_INSIGHTS}
            grid={META_MON_INSIGHTS_GRID}
            highlight={META_MON_INSIGHTS_HIGHLIGHT}
            outcomes={META_MON_INSIGHTS_OUTCOMES}
            titleId="meta-mon-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Список URL и таблица мета — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {META_MON_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Поля в отчёте</h2>
            <p className="mt-2 text-sm text-slate-600">Что снимает мониторинг с каждой страницы.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {META_MON_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">SEO-цепочка в одной платформе.</p>
          ...
          <ModulePlainSection data={META_MON_PLAIN} titleId="meta-mon-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: мониторинг мета-тегов"
              lead="Как отправить URL и прочитать отчёт."
              items={META_MON_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы проверить мета-теги?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — мониторинг мета-тегов и остальные инструменты платформы в одном месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="meta-mon-footer"
