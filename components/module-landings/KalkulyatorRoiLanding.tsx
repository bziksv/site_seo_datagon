...
import {
  ...
  ROI_CALC_HERO,
  ROI_CALC_INSIGHTS,
  ROI_CALC_INSIGHTS_GRID,
  ROI_CALC_INSIGHTS_HIGHLIGHT,
  ROI_CALC_INSIGHTS_OUTCOMES,
  ROI_CALC_OPTIONS,
  ROI_CALC_PLAIN,
  ROI_CALC_SCREENSHOTS,
  ROI_CALC_SLICES,
  ROI_CALC_STATS,
  ROI_CALC_STEPS,
  ROI_CALC_TECH_LAYERS,
  ROI_CALC_TECH_SECTION,
  ROI_CALC_VIDEOS,
} from "@/lib/content/kalkulyator-roi-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function KalkulyatorRoiLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : ROI_CALC_FEATURES;
  ...
  return (
    <>
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{ROI_CALC_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{ROI_CALC_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={ROI_CALC_SCREENSHOTS[0].src}
                      alt={ROI_CALC_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={ROI_CALC_TECH_SECTION}
        layers={ROI_CALC_TECH_LAYERS}
        titleId="roi-calc-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От данных РК до прогноза трафика
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {ROI_CALC_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Что считает калькулятор</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре блока — от окупаемости до прогноза по бюджету. Один экран вместо разрозненных
              формул в таблице.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {ROI_CALC_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={ROI_CALC_INSIGHTS}
            grid={ROI_CALC_INSIGHTS_GRID}
            highlight={ROI_CALC_INSIGHTS_HIGHLIGHT}
            outcomes={ROI_CALC_INSIGHTS_OUTCOMES}
            titleId="roi-calc-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Расчёт по факту и прогноз — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {ROI_CALC_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Режимы и поля</h2>
            <p className="mt-2 text-sm text-slate-600">Что можно указать перед расчётом.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {ROI_CALC_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Маркетинг и аналитика в одной платформе.</p>
          ...
          <ModulePlainSection data={ROI_CALC_PLAIN} titleId="roi-calc-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: калькулятор ROI"
              lead="Как заполнить поля и получить метрики кампании."
              items={ROI_CALC_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы посчитать окупаемость РК?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — калькулятор ROI и остальные инструменты платформы в одном
                месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="roi-calc-footer"
