...
import {
  ...
  SITE_MON_HERO,
  SITE_MON_INSIGHTS,
  SITE_MON_INSIGHTS_GRID,
  SITE_MON_INSIGHTS_HIGHLIGHT,
  SITE_MON_INSIGHTS_OUTCOMES,
  SITE_MON_OPTIONS,
  SITE_MON_PLAIN,
  SITE_MON_SCREENSHOTS,
  SITE_MON_SLICES,
  SITE_MON_STATS,
  SITE_MON_STEPS,
  SITE_MON_TECH_LAYERS,
  SITE_MON_TECH_SECTION,
  SITE_MON_VIDEOS,
} from "@/lib/content/monitoring-saytov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function MonitoringSaytovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : SITE_MON_FEATURES;
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{SITE_MON_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{SITE_MON_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={SITE_MON_SCREENSHOTS[0].src}
                      alt={SITE_MON_SCREENSHOTS[0].caption}
                      fill
      ...
      <div className="min-w-0 max-w-full overflow-x-clip bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          ...
          <ModuleInsightsSection
            meta={SITE_MON_INSIGHTS}
            grid={SITE_MON_INSIGHTS_GRID}
            highlight={SITE_MON_INSIGHTS_HIGHLIGHT}
            outcomes={SITE_MON_INSIGHTS_OUTCOMES}
            titleId="site-mon-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Проекты, статусы и детали — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {SITE_MON_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Параметры проверки</h2>
            <p className="mt-2 text-sm text-slate-600">Что задаётся при создании проекта.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {SITE_MON_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Технический контроль в одной платформе.</p>
          ...
          <ModulePlainSection data={SITE_MON_PLAIN} titleId="site-mon-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: мониторинг сайтов"
              lead="Как добавить проект и настроить оповещения."
              items={SITE_MON_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы контролировать uptime?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — мониторинг сайтов и остальные инструменты платформы в одном месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="site-mon-footer"
