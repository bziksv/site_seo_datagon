...
import {
  ...
  DEDUP_INSIGHTS,
  DEDUP_INSIGHTS_GRID,
  DEDUP_INSIGHTS_HIGHLIGHT,
  DEDUP_INSIGHTS_OUTCOMES,
  DEDUP_OPTIONS,
  DEDUP_PLAIN,
  DEDUP_SCREENSHOTS,
  DEDUP_SLICES,
  ...
  DEDUP_TECH_LAYERS,
  DEDUP_TECH_SECTION,
} from "@/lib/content/udalenie-dublikatov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function UdalenieDublikatovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : DEDUP_FEATURES;
  ...
  return (
    <>
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{DEDUP_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{DEDUP_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[4/3] min-h-[240px] w-full sm:min-h-[300px] lg:min-h-[360px] xl:min-h-[400px]">
                    <Image
                      src={DEDUP_SCREENSHOTS[0].src}
                      alt={DEDUP_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={DEDUP_TECH_SECTION}
        layers={DEDUP_TECH_LAYERS}
        titleId="dedup-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От вставки списка до уникальных строк
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {DEDUP_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            ...
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре типовых задачи — от семантики до импорта из таблиц.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {DEDUP_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={DEDUP_INSIGHTS}
            grid={DEDUP_INSIGHTS_GRID}
            highlight={DEDUP_INSIGHTS_HIGHLIGHT}
            outcomes={DEDUP_INSIGHTS_OUTCOMES}
            titleId="dedup-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Поле ввода и блок фильтров — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {DEDUP_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Фильтры модуля</h2>
            <p className="mt-2 text-sm text-slate-600">Отметьте нужные опции перед нажатием «Удалить дубликаты».</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {DEDUP_FILTERS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Списки и текст в одной платформе.</p>
          ...
          <ModulePlainSection data={DEDUP_PLAIN} titleId="dedup-plain-title" />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Почему {SITE.name}</h2>
            <div className="mt-8 grid gap-8 md:grid-cols-3">
              {DEDUP_ADVANTAGES.map((a) => (
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Нужен чистый список без дублей?</h2>
