...
import {
  DOMAIN_REG_ADVANTAGES,
  DOMAIN_REG_FAQ,
  DOMAIN_REG_FEATURES,
  DOMAIN_REG_HERO,
  DOMAIN_REG_INSIGHTS,
  DOMAIN_REG_INSIGHTS_GRID,
  DOMAIN_REG_INSIGHTS_HIGHLIGHT,
  ...
  DOMAIN_REG_STEPS,
  DOMAIN_REG_CHECKS,
  DOMAIN_REG_TECH_LAYERS,
  DOMAIN_REG_TECH_SECTION,
  DOMAIN_REG_VIDEOS,
} from "@/lib/content/otslezhivanie-sroka-registratsii-domenov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function OtslezhivanieSrokaRegistratsiiDomenovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : DOMAIN_REG_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {DOMAIN_REG_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {DOMAIN_REG_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{DOMAIN_REG_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{DOMAIN_REG_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={DOMAIN_REG_SCREENSHOTS[0].src}
                      alt={DOMAIN_REG_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={DOMAIN_REG_TECH_SECTION}
        layers={DOMAIN_REG_TECH_LAYERS}
        titleId="domain-reg-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От добавления домена до уведомления
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {DOMAIN_REG_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            ...
            <p className="mt-3 max-w-2xl text-slate-600">
              SEO, агентства и владельцы портфеля доменов — один инструмент контроля.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {DOMAIN_REG_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={DOMAIN_REG_INSIGHTS}
            grid={DOMAIN_REG_INSIGHTS_GRID}
            highlight={DOMAIN_REG_INSIGHTS_HIGHLIGHT}
            outcomes={DOMAIN_REG_INSIGHTS_OUTCOMES}
            titleId="domain-reg-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Добавление доменов и таблица отслеживания — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {DOMAIN_REG_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">При регулярной проверке WHOIS и DNS.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {DOMAIN_REG_CHECKS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Домены, доступность и ссылки — в одной платформе.</p>
          ...
          <ModulePlainSection data={DOMAIN_REG_PLAIN} titleId="domain-reg-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: отслеживание доменов"
              lead="Как добавить домены и читать таблицу отслеживания."
              items={DOMAIN_REG_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Нужен контроль доменов?</h2>
