...
import {
  LIST_COMPARE_ADVANTAGES,
  LIST_COMPARE_FAQ,
  LIST_COMPARE_FEATURES,
  LIST_COMPARE_HERO,
  LIST_COMPARE_INSIGHTS,
  LIST_COMPARE_INSIGHTS_GRID,
  LIST_COMPARE_INSIGHTS_HIGHLIGHT,
  LIST_COMPARE_INSIGHTS_OUTCOMES,
  LIST_COMPARE_OPTIONS,
  LIST_COMPARE_PLAIN,
  LIST_COMPARE_SCREENSHOTS,
  LIST_COMPARE_SLICES,
  LIST_COMPARE_STATS,
  LIST_COMPARE_STEPS,
  LIST_COMPARE_TECH_LAYERS,
  LIST_COMPARE_TECH_SECTION,
  LIST_COMPARE_VIDEOS,
} from "@/lib/content/sravnenie-spiskov-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function SravnenieSpiskovLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : LIST_COMPARE_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {LIST_COMPARE_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {LIST_COMPARE_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{LIST_COMPARE_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{LIST_COMPARE_HERO.sub}</p>
              ...
              <div className="mt-10">
                <ModuleLeadCta
                  variant="hero"
                  idPrefix="list-compare-hero"
                  title="Сравнить списки"
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={LIST_COMPARE_SCREENSHOTS[0].src}
                      alt={LIST_COMPARE_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={LIST_COMPARE_TECH_SECTION}
        layers={LIST_COMPARE_TECH_LAYERS}
        titleId="list-compare-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От двух списков до пересечения фраз
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {LIST_COMPARE_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Четыре типа сравнения</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Один режим за прогон — пересечение, уникальные списки или полное объединение без ручной
              сводной таблицы.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {LIST_COMPARE_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={LIST_COMPARE_INSIGHTS}
            grid={LIST_COMPARE_INSIGHTS_GRID}
            highlight={LIST_COMPARE_INSIGHTS_HIGHLIGHT}
            outcomes={LIST_COMPARE_INSIGHTS_OUTCOMES}
            titleId="list-compare-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Два столбца и окно результата — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {LIST_COMPARE_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Что доступно после сравнения.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {LIST_COMPARE_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Семантика и чистка списков в одной платформе.</p>
          ...
          <ModulePlainSection data={LIST_COMPARE_PLAIN} titleId="list-compare-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: сравнение списков"
              lead="Как заполнить столбцы и получить пересечение."
              items={LIST_COMPARE_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы сверить два списка фраз?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — сравнение списков и остальные инструменты платформы в одном
                месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="list-compare-footer"
