...
import {
  ...
  PW_GEN_HERO,
  PW_GEN_INSIGHTS,
  PW_GEN_INSIGHTS_GRID,
  PW_GEN_INSIGHTS_HIGHLIGHT,
  PW_GEN_INSIGHTS_OUTCOMES,
  PW_GEN_OPTIONS,
  PW_GEN_PLAIN,
  PW_GEN_SCREENSHOTS,
  PW_GEN_SLICES,
  ...
  PW_GEN_TECH_LAYERS,
  PW_GEN_TECH_SECTION,
} from "@/lib/content/generator-paroley-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function GeneratorParoleyLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : PW_GEN_FEATURES;
  ...
  return (
    <>
        ...
        <div className="relative z-10 mx-auto min-w-0 max-w-6xl px-4 py-12 md:py-16 lg:py-20">
          ...
          <div className="grid min-w-0 items-center gap-10 lg:grid-cols-2 lg:gap-12 xl:gap-16">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{PW_GEN_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{PW_GEN_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={PW_GEN_SCREENSHOTS[0].src}
                      alt={PW_GEN_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection section={PW_GEN_TECH_SECTION} layers={PW_GEN_TECH_LAYERS} titleId="pw-gen-tech-title" />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От настроек до готового пароля
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {PW_GEN_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Настройки генерации</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре сценария — от PIN до сложного пароля для админки и корпоративной почты.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {PW_GEN_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={PW_GEN_INSIGHTS}
            grid={PW_GEN_INSIGHTS_GRID}
            highlight={PW_GEN_INSIGHTS_HIGHLIGHT}
            outcomes={PW_GEN_INSIGHTS_OUTCOMES}
            titleId="pw-gen-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Форма настроек и выдача паролей — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {PW_GEN_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Параметры</h2>
            <p className="mt-2 text-sm text-slate-600">Что можно задать перед генерацией.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {PW_GEN_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Инструменты команды в одной платформе.</p>
          ...
          <ModulePlainSection data={PW_GEN_PLAIN} titleId="pw-gen-plain-title" />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Почему {SITE.name}</h2>
            <div className="mt-8 grid gap-8 md:grid-cols-3">
              {PW_GEN_ADVANTAGES.map((a) => (
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Нужен надёжный пароль?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — генератор и остальные инструменты платформы в одном месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="pw-gen-footer"
