...
import {
  HTML_EDITOR_ADVANTAGES,
  HTML_EDITOR_CAPABILITIES,
  HTML_EDITOR_FAQ,
  HTML_EDITOR_FEATURES,
  HTML_EDITOR_HERO,
  HTML_EDITOR_INSIGHTS,
  HTML_EDITOR_INSIGHTS_GRID,
  HTML_EDITOR_INSIGHTS_HIGHLIGHT,
  HTML_EDITOR_INSIGHTS_OUTCOMES,
  HTML_EDITOR_OPTIONS,
  HTML_EDITOR_PLAIN,
  HTML_EDITOR_SCREENSHOTS,
  HTML_EDITOR_STATS,
  HTML_EDITOR_STEPS,
  HTML_EDITOR_TECH_LAYERS,
  HTML_EDITOR_TECH_SECTION,
  HTML_EDITOR_VIDEOS,
} from "@/lib/content/html-redaktor-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function HtmlRedaktorLanding({ module }: Props) {
  const featurePills = module.features?.length ? module.features : HTML_EDITOR_FEATURES;
  ...
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {HTML_EDITOR_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {HTML_EDITOR_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{HTML_EDITOR_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{HTML_EDITOR_HERO.sub}</p>
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={HTML_EDITOR_SCREENSHOTS[0].src}
                      alt={HTML_EDITOR_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={HTML_EDITOR_TECH_SECTION}
        layers={HTML_EDITOR_TECH_LAYERS}
        titleId="html-editor-tech-title"
      />
        ...
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От черновика до HTML за четыре шага
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {HTML_EDITOR_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Возможности редактора</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Структура, оформление, медиа и проекты — всё в одном инструменте, без переключения между
              сервисами.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {HTML_EDITOR_CAPABILITIES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={HTML_EDITOR_INSIGHTS}
            grid={HTML_EDITOR_INSIGHTS_GRID}
            highlight={HTML_EDITOR_INSIGHTS_HIGHLIGHT}
            outcomes={HTML_EDITOR_INSIGHTS_OUTCOMES}
            titleId="html-editor-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Редактор и просмотр HTML — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {HTML_EDITOR_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Инструменты панели</h2>
            <p className="mt-2 text-sm text-slate-600">Типовые задачи копирайтера и маркетолога.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {HTML_EDITOR_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">После вёрстки — проверьте текст и мета перед публикацией.</p>
          ...
          <ModulePlainSection data={HTML_EDITOR_PLAIN} titleId="html-editor-plain-title" />

          <section className="mt-16 rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-6 md:p-10">
            <ModuleVideoGallery
              title="Видео: обзор редактора"
              lead="Как ввести текст, оформить блок и получить HTML."
              items={HTML_EDITOR_VIDEOS.map((v) => ({
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы собрать HTML без боли разметки?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — редактор и остальные инструменты платформы в одном месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="html-editor-footer"
