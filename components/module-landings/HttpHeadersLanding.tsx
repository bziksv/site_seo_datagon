...
import {
  ...
  HTTP_HEADERS_INSIGHTS_GRID,
  HTTP_HEADERS_INSIGHTS_HIGHLIGHT,
  HTTP_HEADERS_INSIGHTS_OUTCOMES,
  HTTP_HEADERS_OPTIONS,
  HTTP_HEADERS_PLAIN,
  HTTP_HEADERS_SCREENSHOTS,
  HTTP_HEADERS_SLICES,
  HTTP_HEADERS_STATS,
  HTTP_HEADERS_STEPS,
  HTTP_HEADERS_TECH_LAYERS,
  HTTP_HEADERS_TECH_SECTION,
} from "@/lib/content/http-headers-page";
import { LK_URL, SITE } from "@/lib/site";
import type { ModulePage } from "@/lib/content/modules";

type Props = { module: ModulePage };

export function HttpHeadersLanding({ module }: Props) {
  return (
    <>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-50">
                  {HTTP_HEADERS_HERO.badge}
                </span>
              </div>

              <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight md:text-4xl lg:text-[2.75rem]">
                {HTTP_HEADERS_HERO.title}
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-relaxed text-brand-50/95">{HTTP_HEADERS_HERO.lead}</p>
              <p className="mt-3 font-mono text-sm text-white/70">{HTTP_HEADERS_HERO.sub}</p>
              ...
              <div className="mt-10">
                <ModuleLeadCta
                  variant="hero"
                  idPrefix="http-headers-hero"
                  title="Проверить заголовки"
              ...
              <HeroParallaxMedia>
                <figure className="relative overflow-hidden rounded-2xl border border-white/20 bg-white shadow-xl shadow-black/25">
                  <div className="relative aspect-[16/11] min-h-[220px] w-full sm:min-h-[280px] lg:min-h-[340px] xl:min-h-[380px]">
                    <Image
                      src={HTTP_HEADERS_SCREENSHOTS[0].src}
                      alt={HTTP_HEADERS_SCREENSHOTS[0].caption}
                      fill
      ...
      <ModuleTechSection
        section={HTTP_HEADERS_TECH_SECTION}
        layers={HTTP_HEADERS_TECH_LAYERS}
        titleId="http-headers-tech-title"
      />
      ...
      <div className="min-w-0 max-w-full overflow-x-clip bg-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-14 md:py-20">
          <section>
            ...
            <h2 className="mt-2 text-center text-2xl font-bold text-slate-900 md:text-3xl">
              От URL до отчёта по заголовкам
            </h2>
            <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {HTTP_HEADERS_STEPS.map((item) => (
                <li
                  key={item.step}
          ...
          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Что проверяем в ответе</h2>
            <p className="mt-3 max-w-2xl text-slate-600">
              Четыре направления — от кодов состояния до заголовков безопасности. Одна таблица вместо
              ручного разбора в консоли.
            </p>
            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {HTTP_HEADERS_SLICES.map((z, i) => (
                <div
                  key={z.mode}
          ...
          <ModuleInsightsSection
            meta={HTTP_HEADERS_INSIGHTS}
            grid={HTTP_HEADERS_INSIGHTS_GRID}
            highlight={HTTP_HEADERS_INSIGHTS_HIGHLIGHT}
            outcomes={HTTP_HEADERS_INSIGHTS_OUTCOMES}
            titleId="http-headers-insights-title"
          />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Интерфейс модуля</h2>
            <p className="mt-2 text-slate-600">Один URL и пакетная проверка — как в личном кабинете.</p>
            <div className="mt-8 grid min-w-0 gap-8 sm:grid-cols-2">
              {HTTP_HEADERS_SCREENSHOTS.map((shot) => (
                <figure
                  key={shot.src}
          ...
          <section className="mt-20 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <h2 className="text-xl font-bold text-slate-900">Параметры запроса</h2>
            <p className="mt-2 text-sm text-slate-600">Тонкая настройка перед проверкой.</p>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {HTTP_HEADERS_OPTIONS.map((line) => (
          ...
          <section className="mt-12 rounded-2xl border border-brand-100 bg-brand-50/50 p-6 md:p-8">
            ...
            <p className="mt-2 text-sm text-slate-600">Технический контроль сайта в одной платформе.</p>
          ...
          <ModulePlainSection data={HTTP_HEADERS_PLAIN} titleId="http-headers-plain-title" />

          <section className="mt-20">
            <h2 className="text-2xl font-bold text-slate-900">Почему {SITE.name}</h2>
            <div className="mt-8 grid gap-8 md:grid-cols-3">
              {HTTP_HEADERS_ADVANTAGES.map((a) => (
          ...
          <section className="mt-16 grid gap-8 overflow-hidden rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 p-8 text-white md:grid-cols-2 md:p-12">
            <div>
              <h2 className="text-2xl font-bold">Готовы проверить ответ сервера?</h2>
              <p className="mt-3 text-brand-100">
                Зарегистрируйтесь в кабинете — модуль HTTP-заголовков и остальные инструменты платформы в
                одном месте.
              </p>
            </div>
            <ModuleLeadCta
              variant="card"
              idPrefix="http-headers-footer"
