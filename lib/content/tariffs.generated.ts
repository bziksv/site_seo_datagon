/** AUTO-GENERATED: node scripts/scrape-tariffs.mjs */
export type TariffPlan = {
  id: string;
  name: string;
  /** @deprecated Используйте pricePerDay — оставлено для обратной совместимости */
  price: string;
  /** Стоимость тарифа в рублях за календарный день (как в ЛК) */
  pricePerDay: number;
  priceNote?: string;
  tagline: string;
  features: string[];
  highlighted?: boolean;
  badge?: string;
};

export const TARIFF_DISCOUNT_NOTE =
  "Итоговая сумма зависит от периода оплаты. Скидки при оплате на 3 месяца — 10%, на 6 месяцев — 20%, на 12 месяцев — 35%.";

export const TARIFF_PLANS: TariffPlan[] = [
  {
    id: "Free",
    name: "Бесплатный",
    price: "0 ₽",
    pricePerDay: 0,
    tagline: "Попробовать платформу и утилиты",
    highlighted: false,
    features: [
      "Анализатор Релевантности Страницы: 3 проверки в месяц",
      "Анализ текста: 3 проверки в месяц",
      "Анализ конкурентов по ключевым словам: 3 проверки в месяц",
      "Проверка индексации страниц (Яндекс и Google): 5 проверок в месяц",
      "Проверка текста Есенин: 5 проверок в месяц",
      "Кластеризатор ключевых слов: 50 условных запросов в месяц",
      "Мониторинг сайтов: 1 проект (без email-оповещений; Telegram при подключении бота)",
      "Отслеживание срока регистрации доменов: 1 домен (без email-оповещений; Telegram при подключении бота)",
      "Мониторинг Мета-Тегов: 1 проект, 10 страниц",
      "Генератор слов: без ограничений",
      "Генератор паролей: без ограничений",
      "Подсчет длины текста: без ограничений",
      "Сравнение списков ключевых фраз: без ограничений",
      "Выделение уникальных слов в тексте: без ограничений",
      "Визуальный HTML-редактор: без ограничений",
      "Удаление дубликатов: без ограничений",
      "Генератор UTM-меток: без ограничений",
      "Калькулятор ROI: без ограничений",
      "Отслеживание размещенных ссылок на сайтах: 1 проект, 20 ссылок (без email-оповещений; Telegram при подключении бота)",
      "Мониторинг позиций сайта: 50 проверок в месяц",
    ],
  },
  {
    id: "Optimal",
    name: "Оптимальный",
    price: "65 ₽",
    pricePerDay: 65,
    tagline: "Фрилансер или небольшой проект",
    highlighted: false,
    features: [
      "Анализатор Релевантности Страницы: 600 запросов в месяц",
      "Анализ текста: 600 запросов в месяц",
      "Анализ конкурентов по ключевым словам: 300 запросов в месяц",
      "Проверка индексации страниц (Яндекс и Google): 600 проверок в месяц",
      "Проверка текста Есенин: 100 проверок в месяц",
      "Мониторинг сайтов на доступность: 10 проектов",
      "Отслеживание срока регистрации доменов: 10 проектов",
      "Мониторинг Мета-Тегов: 10 проектов, 1000 страниц",
      "Генератор слов: без ограничений",
      "Генератор паролей: без ограничений",
      "Подсчет длины текста: без ограничений",
      "Сравнение списков ключевых фраз: без ограничений",
      "Выделение уникальных слов в тексте: без ограничений",
      "Визуальный HTML-редактор: без ограничений",
      "Удаление дубликатов: без ограничений",
      "Генератор UTM-меток: без ограничений",
      "Калькулятор ROI: без ограничений",
      "Отслеживание размещенных ссылок на сайтах: 10 проектов, 200 ссылок",
      "Мониторинг позиций сайта: 20 000 проверок в месяц",
    ],
  },
  {
    id: "Ultimate",
    name: "Ультимат",
    price: "129 ₽",
    pricePerDay: 129,
    tagline: "Агентство и несколько клиентов",
    highlighted: true,
    badge: "Популярный",
    features: [
      "Анализатор Релевантности Страницы: 1500 запросов в месяц",
      "Анализ текста: 1500 запросов в месяц",
      "Анализ конкурентов по ключевым словам: 1500 запросов в месяц",
      "Проверка индексации страниц (Яндекс и Google): 1 500 проверок в месяц",
      "Проверка текста Есенин: 300 проверок в месяц",
      "Мониторинг сайтов на доступность: 30 проектов",
      "Отслеживание срока регистрации доменов: 30 проектов",
      "Мониторинг Мета-Тегов: 20 проектов, 2000 страниц",
      "Генератор слов: без ограничений",
      "Генератор паролей: без ограничений",
      "Подсчет длины текста: без ограничений",
      "Сравнение списков ключевых фраз: без ограничений",
      "Выделение уникальных слов в тексте: без ограничений",
      "Визуальный HTML-редактор: без ограничений",
      "Удаление дубликатов: без ограничений",
      "Генератор UTM-меток: без ограничений",
      "Калькулятор ROI: без ограничений",
      "Отслеживание размещенных ссылок на сайтах: 20 проектов, 400 ссылок",
      "Мониторинг позиций сайта: 50 000 проверок в месяц",
    ],
  },
  {
    id: "Maximum",
    name: "Максимум",
    price: "194 ₽",
    pricePerDay: 194,
    tagline: "Крупные объёмы и много проектов",
    highlighted: false,
    features: [
      "Анализатор Релевантности Страницы: 2400 запросов в месяц",
      "Анализ текста: 2400 запросов в месяц",
      "Анализ конкурентов по ключевым словам: 2400 запросов в месяц",
      "Проверка индексации страниц (Яндекс и Google): 2 400 проверок в месяц",
      "Проверка текста Есенин: 700 проверок в месяц",
      "Мониторинг сайтов на доступность: 100 проектов",
      "Отслеживание срока регистрации доменов: 100 проектов",
      "Мониторинг Мета-Тегов: 50 проектов, 5000 страниц",
      "Генератор слов: без ограничений",
      "Генератор паролей: без ограничений",
      "Подсчет длины текста: без ограничений",
      "Сравнение списков ключевых фраз: без ограничений",
      "Выделение уникальных слов в тексте: без ограничений",
      "Визуальный HTML-редактор: без ограничений",
      "Удаление дубликатов: без ограничений",
      "Генератор UTM-меток: без ограничений",
      "Калькулятор ROI: без ограничений",
      "Отслеживание размещенных ссылок на сайтах: 50 проектов, 5 000 ссылок",
      "Мониторинг позиций сайта: 120 000 проверок в месяц",
    ],
  },
];

export const TARIFF_COMPARE_ROWS = [
  { label: "Анализ релевантности, запросов/мес", key: "relevance" as const },
  { label: "Анализ текста, запросов/мес", key: "text" as const },
  { label: "Анализ конкурентов, запросов/мес", key: "competitors" as const },
  { label: "Проверка индексации, проверок/мес", key: "indexCheck" as const },
  { label: "Проверка текста Есенин, проверок/мес", key: "eseninTextCheck" as const },
  { label: "Кластеризатор, усл. запросов/мес", key: "cluster" as const },
  { label: "Мониторинг позиций, проверок/мес", key: "positions" as const },
  { label: "Мониторинг сайтов, проектов", key: "sites" as const },
  { label: "Мониторинг сайтов: оповещения", key: "sitesAlerts" as const },
  { label: "Отслеживание срока регистрации, доменов", key: "domains" as const },
  { label: "Отслеживание срока регистрации: оповещения", key: "domainsAlerts" as const },
  { label: "Мета-теги, проектов / страниц", key: "meta" as const },
  { label: "Отслеживание ссылок, проектов / ссылок", key: "links" as const },
  { label: "Отслеживание ссылок: оповещения", key: "linksAlerts" as const },
] as const;

export const TARIFF_COMPARE: Record<string, Record<string, string>> = {
  relevance: { Free: "3", Optimal: "600", Ultimate: "1 500", Maximum: "2 400" },
  text: { Free: "3", Optimal: "600", Ultimate: "1 500", Maximum: "2 400" },
  competitors: { Free: "3", Optimal: "300", Ultimate: "1 500", Maximum: "2 400" },
  indexCheck: { Free: "5", Optimal: "600", Ultimate: "1 500", Maximum: "2 400" },
  eseninTextCheck: { Free: "5", Optimal: "100", Ultimate: "300", Maximum: "700" },
  cluster: { Free: "50", Optimal: "5 000", Ultimate: "12 000", Maximum: "30 000" },
  positions: { Free: "50", Optimal: "20 000", Ultimate: "50 000", Maximum: "120 000" },
  sites: { Free: "1", Optimal: "10", Ultimate: "30", Maximum: "100" },
  sitesAlerts: { Free: "telegram-only", Optimal: "email-telegram", Ultimate: "email-telegram", Maximum: "email-telegram" },
  domains: { Free: "1", Optimal: "10", Ultimate: "30", Maximum: "100" },
  domainsAlerts: { Free: "telegram-only", Optimal: "email-telegram", Ultimate: "email-telegram", Maximum: "email-telegram" },
  meta: { Free: "1 / 10", Optimal: "10 / 1 000", Ultimate: "20 / 2 000", Maximum: "50 / 5 000" },
  links: { Free: "1 / 20", Optimal: "10 / 200", Ultimate: "20 / 400", Maximum: "50 / 5 000" },
  linksAlerts: { Free: "telegram-only", Optimal: "email-telegram", Ultimate: "email-telegram", Maximum: "email-telegram" },
};
